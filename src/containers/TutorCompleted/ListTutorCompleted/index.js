...
ListTutorCompleted.propTypes = {};

function ListTutorCompleted(props) {
  ...
  useEffect(() => {
    dispatch(getListTutorCompleted());
  }, []);

  const tutors = useSelector((state) => state.tutors.listTutorCompleted);
  ...
  const handleTutorViewClick = (tutor) => {
    ...
    const viewTutorUrl = `/tutorview/${tutor.id}`;
  ...
  return (
    <>
      <Layout>
        <h1 className="text-center">List Completed Tutor </h1>
        <div className="row">
...
export default ListTutorCompleted;
