...
import { acceptPost, deletePost, denyPost, getListPostMustVerify } from "../../../actions";
import Layout from "../../../components/Layout";
import Pagination from "../../../components/Pagination";
import PostItem from "./PostItem";


ListPostMustVerify.propTypes = {};

function ListPostMustVerify(props) {
  ...
  const posts = useSelector((state) => state.posts.listPostMustVerify);
  ...
  const handlePostViewClick = (post) => {
    ...
    const viewPostUrl = `/postview-must-verify/${post.id}`;
  ...
  const handlePostAcceptClick = async (post) => {
  ...
  const handlePostDenyClick = async (post) => {
    console.log("deny: ", post);
    await dispatch(denyPost({ id: post.id, approval: false }));
  };

  return (
    <>
     <Layout>
        <h1 className="text-center">Verify Post</h1>
        <div className="row">
                        ...
                        <tbody data-test="table-body">
                          {currentCustomers?.map((post) => (
                            <PostItem
                              ...
                              onAcceptClick={handlePostAcceptClick}
                              onDenyClick={handlePostDenyClick}
                            />
                          ))}
                        </tbody>
                      </table>
...
export default ListPostMustVerify;
